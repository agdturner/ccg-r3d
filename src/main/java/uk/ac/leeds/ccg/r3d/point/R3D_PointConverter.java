...
//    }
    ...
    public static void rotateAxisX(R3D_Point p, R3D_PointDoubleOffset pd, boolean CW, double radians, int oom) {
        if (p instanceof R3D_MorphPoint) {
            R3D_MorphPoint morphPoint = (R3D_MorphPoint) p;
            rawRotateAxisX(morphPoint.p1, pd, CW, radians, oom);
            rawRotateAxisX(morphPoint.p2, pd, CW, radians, oom);
        }
        rawRotateAxisX(p, pd, CW, radians, oom);
    }

    public static void rotateAxisY(R3D_Point p, R3D_PointDoubleOffset pd, boolean CW, double radians, int oom) {
        if (p instanceof R3D_MorphPoint) {
            R3D_MorphPoint morphPoint = (R3D_MorphPoint) p;
            rawRotateAxisY(morphPoint.p1, pd, CW, radians, oom);
            rawRotateAxisY(morphPoint.p2, pd, CW, radians, oom);
        }
        rawRotateAxisY(p, pd, CW, radians, oom);
    }

    public static void rotateAxisZ(R3D_Point p, R3D_PointDoubleOffset pd, boolean CW, double radians, int oom) {
        if (p instanceof R3D_MorphPoint) {
            R3D_MorphPoint morphPoint = (R3D_MorphPoint) p;
            rawRotateAxisZ(morphPoint.p1, pd, CW, radians, oom);
            rawRotateAxisZ(morphPoint.p2, pd, CW, radians, oom);
        }
        rawRotateAxisZ(p, pd, CW, radians, oom);
    }

    private static void rawRotateAxisX(R3D_Point p, R3D_PointDoubleOffset pd, boolean CW, double radians, int oom) {
        double radius = Math.sqrt(pd.y * pd.y + pd.z * pd.z);
        double theta = Math.atan2(pd.z, pd.y) - radians;
//        p.v = new V3D_Vector(
//                p.v.getDX(oom),
...
//                oom);
    ...
    private static void rawRotateAxisY(R3D_Point p, R3D_PointDoubleOffset pd, boolean CW,  double radians, int oom) {
        double radius = Math.sqrt(pd.x * pd.x + pd.z * pd.z);
        double theta = Math.atan2(pd.x, pd.z) - radians * (CW ? -1 : 1);;
//        p.v = new V3D_Vector(
...
//                oom);
    ...
    private static void rawRotateAxisZ(R3D_Point p, R3D_PointDoubleOffset pd, boolean CW, double radians, int oom) {
        double radius = Math.sqrt(pd.y * pd.y + pd.x * pd.x);
        double theta = Math.atan2(pd.y, pd.x) - radians;
        p.v = new V3D_Vector(
