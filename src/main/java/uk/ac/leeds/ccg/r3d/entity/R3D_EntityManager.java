...
//                Math_BigRational.ZERO, oom));
    ...
    public void update() {
        ...
        if (this.mouse.getButton() == ClickType.LeftClick) {
            int xDif = x - initialX;
            int yDif = y - initialY;
            this.rotate(0, Math.toRadians(-yDif / mouseSensitivity), Math.toRadians(-xDif / mouseSensitivity), true);
            //System.out.println("Left click");
        } else if (this.mouse.getButton() == ClickType.RightClick) {
            int xDif = x - initialX;
            this.rotate(Math.toRadians(-xDif / mouseSensitivity), 0, 0, true);
    ...
    private void rotate(double xRadians, double yRadians, double zRadians, boolean CW) {
        this.entities.forEach(e -> {
            e.rotate(xRadians, yRadians, zRadians, CW, this.lightVector);
        });
    }
