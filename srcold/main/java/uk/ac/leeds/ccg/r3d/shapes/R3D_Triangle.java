...
public class R3D_Triangle implements Serializable {
    ...
    public void rotate(double xDegrees, double yDegrees, double zDegrees, V3D_Vector lightVector) {
        for (var point : points) {
            R3D_PointDoubleOffset pd = point.getDouble(oom);
            R3D_PointConverter.rotateAxisX(point, pd, xDegrees, oom);
            R3D_PointConverter.rotateAxisY(point, pd, yDegrees, oom);
            R3D_PointConverter.rotateAxisZ(point, pd, zDegrees, oom);
        }
        this.updateVisibility();
